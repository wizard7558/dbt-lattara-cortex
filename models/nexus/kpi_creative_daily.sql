...
linkedin_dates AS (
  ...
  FROM `res-analytics.linkedin_ads.ad_analytics_by_creative`
  CROSS JOIN inputs
...
linkedin_conversions AS (
  ...
  FROM `res-analytics.linkedin_ads.ad_analytics_by_creative_with_conversion_breakdown` cb
  LEFT JOIN `res-analytics.linkedin_ads.conversion_history` ch
...
linkedin_base AS (
  ...
  FROM `res-analytics.linkedin_ads.ad_analytics_by_creative` la
  CROSS JOIN inputs
  ...
  INNER JOIN (
    ...
    FROM `res-analytics.linkedin_ads.creative_history`
  ) crh
  ...
  INNER JOIN (
    ...
    FROM `res-analytics.linkedin_ads.campaign_history`
  ) cmh
...
tiktok_dates AS (
  ...
  FROM `res-analytics.tiktok_ads.ad_report_hourly`
  CROSS JOIN inputs
  GROUP BY ad_id, inputs.at_date
),
...
tiktok_base AS (
  ...
  FROM `res-analytics.tiktok_ads.ad_report_hourly` tr
  INNER JOIN (
    SELECT
      ad_id,
      adgroup_id,
    ...
    FROM `res-analytics.tiktok_ads.ad_history`
  ) ah
...
bing_dates AS (
  ...
  FROM `res-analytics.bingads.ad_performance_daily_report`
  CROSS JOIN inputs
  GROUP BY ad_id, inputs.at_date
),
...
bing_conversions AS (
  ...
  FROM `res-analytics.bingads.destination_url_performance_daily_report`
...
bing_base AS (
  ...
  FROM `res-analytics.bingads.ad_performance_daily_report` br
  CROSS JOIN inputs
